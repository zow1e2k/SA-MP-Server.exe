...
{
    if (mainInfo[playerid][logged] == false) return true;
    if (mainInfo[playerid][member] == 2 && mainInfo[playerid][rank] != 16 ||
    mainInfo[playerid][member] == 3 && mainInfo[playerid][rank] != 16 ||
    mainInfo[playerid][member] == 4 && mainInfo[playerid][rank] != 16 ||
    mainInfo[playerid][member] == 6 && mainInfo[playerid][rank] != 6 ||
    mainInfo[playerid][member] == 7 && mainInfo[playerid][rank] != 6 ||
    mainInfo[playerid][member] == 8 && mainInfo[playerid][rank] != 6 ||
    mainInfo[playerid][member] == 9 && mainInfo[playerid][rank] != 6 ||
    mainInfo[playerid][member] == 10 && mainInfo[playerid][rank] != 6)
        return SCM(playerid, COLOR_SYSTEM, "Несуществующая команда");
    ...
    new string[sizeof(str)+MAX_PLAYER_NAME*2+9+22*2-16];
...
CMD:unjail(playerid, params[])
{
    ...
    mainInfo[playerid][member] == 10 && mainInfo[playerid][rank] != 6)
        return SCM(playerid, COLOR_SYSTEM, "Несуществующая команда");
    if (sscanf(params, "is[128]", params[0], params[1])) return SCM(playerid, COLOR_SYSTEM, "Используйте /unjail [id] [причина]");
    if (mainInfo[params[0]][member] != mainInfo[playerid][member]) return SCM(playerid, COLOR_SYSTEM, "Не в Вашей фракции");
    static str[] = "%s %s[%i] вытащил %s %s[%i] из КПЗ по причине: %s";
    new string[sizeof(str)+MAX_PLAYER_NAME*2+6+22*2-14];
...
{
    if (mainInfo[playerid][logged] == false) return true;
    if (mainInfo[playerid][member] == 1 && mainInfo[playerid][rank] != 16 ||
    mainInfo[playerid][member] == 2 && mainInfo[playerid][rank] != 16 ||
    mainInfo[playerid][member] == 3 && mainInfo[playerid][rank] != 16 ||
    mainInfo[playerid][member] == 4 && mainInfo[playerid][rank] != 16 ||
    mainInfo[playerid][member] == 6 && mainInfo[playerid][rank] != 6 ||
    mainInfo[playerid][member] == 7 && mainInfo[playerid][rank] != 6 ||
...
CMD:invite(playerid, params[])
{
    ...
    new string[sizeof(str)+MAX_PLAYER_NAME+3+22-6];
...
CMD:gethealth(playerid, params[])
{
    ...
    format(string,sizeof(string),"%s[%i] HP: %f и %f",playerName(params[0]),params[0],healthP, mainInfo[params[0]][health]);
    SCM(playerid,COLOR_SYSTEM,string);
    return true;
}
CMD:skinfix(playerid, params[])
{
    ...
    if (mainInfo[params[0]][member] == 0) return SCM(playerid, COLOR_SYSTEM, "Игрок не состоит в организации");
    getRankSkin(params[0]);
    SetPlayerSkin(params[0],mainInfo[params[0]][skin]);
    static str[] = "[A] [%s] %s[%i] пофиксил облик игроку %s[%i]";
    new string[sizeof(str)+MAX_PLAYER_NAME*2+6+22-10];
...
CMD:setmember(playerid, params[])
{
    ...
    new string[sizeof(str)+MAX_PLAYER_NAME*2+6+22*3-14];
...
CMD:mute(playerid, params[])
{
    ...
    new string[sizeof(str)+MAX_PLAYER_NAME*2+9+22+64-14]; 
...
CMD:info(playerid, params[])
{
    ...
    format(string,sizeof(string),"{FFFFFF}Позывной\t\t%s\nУровень допуска\t%i\nГруппировка\t\t%s\nЗвание\t\t\t%s\nОпыт\t\t\t%i\nЗамечания\t\t%i\n\
                        Клан\t\t\t%s\nДонат очки\t\t%i\nЧасы в зоне\t\t%i\nVIP уровень\t\t%i\nIP адрес\t\t%s\nДеньги\t\t%i",
                        mainInfo[params[0]][nickName],mainInfo[params[0]][accessLevel],memberName(params[0]),
                        ...
                        mainInfo[params[0]][hoursLevel],mainInfo[params[0]][vipLevel], playerIp(params[0]),mainInfo[params[0]][money]);
